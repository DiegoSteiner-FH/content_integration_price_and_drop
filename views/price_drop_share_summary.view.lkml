view: price_drop_share_summary {
  ...
  filter: live_sources {
    ...
    description: "Which content sources count as 'live' in the If Live panel -- select specific sources to simulate 'what if only these were deployed', or leave empty to consider every source with a profitable candidate (the default/neutral state, matching the row-level Content Source Share tile's own default). Pushed into the derived table's own ranking logic via Looker's condition-tag mechanism -- changing this re-runs the query, since this view computes the comparison entirely in SQL specifically to avoid the per-attempt row-cap ceiling the row-level tile hits at real volume. Why (2026-09-15, DS): PR #59 tried removing this hardcoded list in favor of sql: ${gds} (pointing at this view's own gds dimension purely for live-suggest generation) -- that broke the entire model (every explore in this project 404'd from Looker's own API immediately after deploy). Reverted to a hardcoded suggestions: list as an emergency fix to restore access; voyzant added manually for now. sql: on a filter-only field with no natural backing column apparently isn't valid the way it is on a real dimension -- needs verifying against Looker's actual LookML grammar before trying a dynamic fix here again, rather than assumed from how dimension suggestions work."
  }
