view: price_drop_share_summary {
  ...
  filter: live_sources {
    ...
    label: "Live Sources"
    sql: ${gds} ;;
    description: "Which content sources count as 'live' in the If Live panel -- select specific sources to simulate 'what if only these were deployed', or leave empty to consider every source with a profitable candidate (the default/neutral state, matching the row-level Content Source Share tile's own default). Pushed into the derived table's own ranking logic via Looker's condition-tag mechanism -- changing this re-runs the query, since this view computes the comparison entirely in SQL specifically to avoid the per-attempt row-cap ceiling the row-level tile hits at real volume. Why (2026-09-15, DS): removed the hardcoded suggestions: list (was ['aerohub','tiantai','travelportplus','gtsfly','travelcaster']) after a new source ('voyzant') launched and didn't show up in this dropdown -- same root cause as price_drop_candidates.gds. This filter has no dimension of its own to generate live suggestions from (it's a filter-only field feeding the derived table's condition tags, not a real output column), so unlike that dimension, simply dropping suggestions: here would leave an empty dropdown -- added sql: ${gds} instead, pointing at this view's own gds dimension purely so Looker has something to run its live distinct-values suggest query against. Does not change filtering behavior -- the {% condition %} tags below still target bg.gds explicitly wherever they're used; this sql: is suggestion-generation only, since filter: fields never auto-apply a WHERE clause on their own."
  }
